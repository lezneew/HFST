...
\begin{center}
	\begin{tabular}{|l|p{4cm}|l|p{4cm}|}
		...
		\hline
		\textbf{Versuchsnr.:} & 3 & \textbf{Datum:} & 23.12.2025 \\
		\hline
	\end{tabular}
\end{center}
	
	\vspace{1cm}
	\setcounter{section}{3}  % setzt den Section Zähler auf 1
	\setcounter{subsection}{5}  % setzt den Subsection Zähler auf 5
	\setcounter{subsubsection}{0}  % setzt den Subsubsection Zähler auf 0

	\subsubsection{}
	Figure \ref{fig:resistors} shows the measured resistance $R_s$ and impedance $|Z|$ of a carbon resistor and a coaxial termination over a frequency range between 1 MHz and 30 MHz.
	...
	$|Z| \approx X_L = 2\pi f L \approx 45 \Omega$ with L=2.2 $\mu$H.
	...
	A pronounced maximum of $|Z|$ occurs at $\sim30\,\mathrm{MHz}$, indicating
...
%	transition of approximately $180^\circ$, which is characteristic of quartz resonance.
	...
	is $3\,\mathrm{dB}$ above the minimum. From the figure \ref{fig:353}, a bandwidth of approximately
	...
	\approx 18200.45
	\]
	indicating a very high Q and highly frequency-selective resonator \cite{bohmer2018}.
	...
	\begin{figure}[H]
		\centering
		\input{354.pgf}
		\caption{Measured impedance of a quartz resonator as a function of frequency.}
		\label{fig:354}
	\end{figure}
	...
	impedance to increase rapidly \cite{tietze}.
	...
	\begin{figure}[H]
		\centering
		\input{355.pgf}
		\caption{Measured impedance of an unknown part as a function of frequency.}
		\label{fig:355}
	\end{figure}
	The measured series resistance is 1 $\Omega$. To compare this value to the theoretical value the equation for our reflection at -0.18 dB and 90 degree needs to be evaluated \cite{bangert}:
	...
	\begin{figure}[H]
		\centering
		\input{356.pgf}
		\caption{Measured phase of return loss of two different cables with different lengths a function of frequency.}
		\label{fig:356}
	\end{figure}
	From the formula of the refection coefficient $r = r_L \cdot e^{-j2\beta l}$ and the fact that we can distinctly have maxima at a phase of 180 degree as seen in fig. \ref{fig:356}, $-j2\beta l$ is set to $\pi$. in addition it is known that $\beta = \frac{2\pi}{\lambda}$ which results in a length of $l = \frac{\lambda}{4}$. After considering a velocity factor of 0.66 and the relation $\lambda_0 = \frac{c_0}{f}$ to connect the frequency to the wave length the resulting equation to calculate the lengths of the cables is $l = \frac{c_0 \cdot VF}{4 \cdot f}$ \cite{agilent_vna}. 
	...
	\subsubsection{}
	Figure \ref{fig:357} shows the return loss of a matching network. A resonance can be observed at 14.238 MHz with a loss of -25.51 dB. The SWR is defined as $\frac{1+\rho}{1-\rho}$ \cite{bangert}. With the measured return loss the SWR is 1.11.
	\begin{figure}[H]
		\centering
		\input{357.pgf}
		\caption{Measured Return Loss of Matching Network as a function of frequency.}
		\label{fig:357}
	\end{figure}
	...
	\begin{figure}[H]
		\centering
		\input{358.pgf}
		\caption{Measured return loss of a antenna as a function of frequency.}
		\label{fig:358}
	\end{figure}
	Figure \ref{fig:358} shows a minimum in the reflection coefficient at approximately \(91.9\,\mathrm{MHz}\), where the magnitude is measured as \(-11.85\,\mathrm{dB}\). Substituting this value into the equation below gives the standing-wave ratio (SWR) \cite{bangert}:
	...
	\[
	l = \frac{c_0}{4 f} = \frac{c_0}{4 \cdot 91.601\,\mathrm{MHz}} = 0.818\,\mathrm{m}
	\]
	...
	\begin{figure}[H]
		\centering
		\input{3510.pgf}
		\caption{Transmission loss of three different attenuators}%\url{https://tenor.com/de/view/will-smith-gif-3197096261065537476}{gg}}
		\label{fig:3510}
	\end{figure}
	
	\subsubsection{}
	
	All three attenuators can be used for the entire frequency range of 8 GHz as can be seen in fig. \ref{fig:3510}. The cutoff frequency was given on the elements and is much higher at 18 GHz.
	...
	\begin{figure}[H]
		\centering
		\input{3513.pgf}
		\caption{Measured LC Band-Pass Filter Transmission Loss a function of frequency.}
		\label{fig:3513}
	\end{figure}
	\subsubsection{}
	The  6 dB bandwidth of the IF filter is $\sim 270$ kHz as can be seen in fig. \ref{fig:3514}. It is important to use double the 3 dB threshold because the IF filter only has one filter stage compared with the LC filter.
			\begin{figure}[H]
		\centering
		\input{3514.pgf}
		\caption{Measured IF Filter Transmission Loss as a function of frequency.}
		\label{fig:3514}
	\end{figure}
	\subsubsection{}
	At the center frequency, the LC band-pass filter shows significantly lower insertion loss than the IF filter. The LC filter exhibits a transmission coefficient of -0.18 dB, whereas the IF filter shows -11 dB. At twice the center frequency, where strong attenuation is desired the LC filter attenuates the signal by only -6 dB, while the IF filter provides much stronger suppression. Consequently, the IF filter offers superior out-of-band rejection, whereas the LC band-pass filter provides better transmission at the center frequency.
	\vfill
